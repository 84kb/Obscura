...
export class LegacyMediaLibrary {
  ...
  public async getMediaFileWithDetails(id: number) {
    ...
    let parent = null
    if (media.parentId) {
      const p = this.db.mediaFiles.find(m => m.id === media.parentId)
      if (p) {
        parent = { ...p, thumbnail_path: toAbsolutePath(p.thumbnail_path) }
      }
    }
  ...
  public async updateParentId(childId: number, parentId: number | null) {
    await this.ready
...
export const mediaDB = {
  ...
  updateParentId: async (childId: number, parentId: number | null) => getActiveMediaLibrary()?.updateParentId(childId, parentId),
