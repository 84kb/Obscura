...
import os from 'os'
import { MediaFile, Tag, Folder } from '@obscura/core'
        ...
        try {
            ...
            expressApp.get('/api/media', authMiddleware, requirePermission('READ_ONLY'), async (req: AuthenticatedRequest, res: Response) => {
                try {
                    ...
                    const tags = req.query.tags ? String(req.query.tags).split(',').map(Number) : []
                    const folders = req.query.folders ? String(req.query.folders).split(',').map(Number) : []
                    const ratings = req.query.ratings ? String(req.query.ratings).split(',').map(Number) : []
                    ...
                    if (tags.length > 0) {
                        media = media.filter((m: MediaFile) => m.tags?.some((t: Tag) => tags.includes(t.id)))
                    }
                    if (folders.length > 0) {
                        media = media.filter((m: MediaFile) => m.folders?.some((f: Folder) => folders.includes(f.id)))
                    }
                    if (ratings.length > 0) {
                        media = media.filter((m: MediaFile) => ratings.includes(m.rating || 0))
                    }
                    if (type && type !== 'all') {
                        // MediaFile.file_type is 'video' | 'audio'.
                        // We check if type is one of them.
                        if (type === 'video' || type === 'audio') {
                            media = media.filter((m: MediaFile) => m.file_type === type)
                        }
                    }
