...
import { ServerConfig, RemoteLibrary } from '@obscura/core'
...
import { registerPluginSystem } from './plugin-api'
...
// 日本語環境対応: キャッシュディレクトリをデフォルトの場所へ（Electron標準のuserDataを使用）
...
function createWindow() {
    ...
    // テーマ適用後にページ側から通知を受けて表示（フラッシュ完全防止）
    const showWindow = () => {
        if (mainWindow && !mainWindow.isVisible()) {
            mainWindow.show()
            if (mainWindow) initUpdater(mainWindow)
            if (mainWindow) setupAudioEvents(mainWindow.webContents)
        }
    }
    ...
    // フェイルセーフ: 5秒以内にrenderer-readyが届かなくても表示する
    setTimeout(() => {
        showWindow()
    }, 5000)
...
app.whenReady().then(() => {
    ...
    try {
        ...
        registerPluginSystem(userDataPath)
...
ipcMain.handle('add-media-parent', async (_event, childId: number, parentId: number) => {
    try {
        return mediaDB.addParent(childId, parentId)
    } catch (error) {
        console.error('Failed to add media parent:', error)
        throw error
    }
})

ipcMain.handle('remove-media-parent', async (_event, childId: number, parentId: number) => {
    try {
        return mediaDB.removeParent(childId, parentId)
    } catch (error) {
        console.error('Failed to remove media parent:', error)
        throw error
    }
})
...
ipcMain.handle('get-shared-users', async () => {
    const users = sharedUserDB.getAllUsers()

    // ホストユーザーを仮想的に生成してリストの先頭に追加
    const clientConfig = getClientConfig()
    const hostUser: SharedUser = {
        id: 'host',
        userToken: 'host-token',
        accessToken: 'host-access',
        nickname: clientConfig.nickname || 'Host',
