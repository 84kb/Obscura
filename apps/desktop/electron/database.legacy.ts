...
export class LegacyMediaLibrary {
  ...
  constructor(libraryPath: string) {
    ...
    this.mediaCachePath = path.join(libraryPath, 'media_cache.json')
    ...
    this.ready = this.init().catch(err => {
      console.error(`[MediaLibrary] CRITICAL: Initialization failed for ${libraryPath}`, err)
    })
    console.log(`[MediaLibrary] Constructor finished for ${libraryPath}`)
  }

  private async init() {
    console.log(`[MediaLibrary] Async Init started for ${this.path}`)
    try {
      // 1. Check for legacy database and migrate if needed
      if (await fs.pathExists(this.dbPath) && !(await fs.pathExists(this.tagsPath))) {
        console.log('[MediaLibrary] Legacy database found. Starting migration...')
        this.migrateFromLegacyDatabase() // Keep synchronous as it's a one-time migration
        console.log('[MediaLibrary] Migration completed.')
      }

      // 2. Load global metadata
      if (await fs.pathExists(this.tagsPath)) this.db.tags = await fs.readJson(this.tagsPath)
      if (await fs.pathExists(this.tagGroupsPath)) this.db.tagGroups = await fs.readJson(this.tagGroupsPath)
      if (await fs.pathExists(this.foldersPath)) this.db.folders = await fs.readJson(this.foldersPath)
      if (await fs.pathExists(this.auditLogsPath)) this.db.auditLogs = await fs.readJson(this.auditLogsPath)
      ...
      // 3. メディアメタデータの読み込み
      // キャッシュが存在すれば一括読み込み、なければ個別ディレクトリ走査
      this.db.mediaFiles = []
      this.db.mediaTags = []
      this.db.mediaFolders = []
      this.db.comments = []

      const cacheLoaded = await this.loadFromMediaCache()

      if (!cacheLoaded) {
        // キャッシュなし: 従来の個別ディレクトリ走査にフォールバック
        const imagesDir = path.join(this.path, 'images')
        if (await fs.pathExists(imagesDir)) {
          const dirs = await fs.readdir(imagesDir)

          // EMFILE回避のためバッチ処理で並列読み込み
          const BATCH_SIZE = 50
          const totalDirs = dirs.length

          console.log(`[MediaLibrary] Loading ${totalDirs} media directories...`)

          for (let i = 0; i < totalDirs; i += BATCH_SIZE) {
            const chunk = dirs.slice(i, i + BATCH_SIZE)

            await Promise.all(chunk.map(async (dir: string) => {
              const metaPath = path.join(imagesDir, dir, 'metadata.json')
              try {
                const meta = await fs.readJson(metaPath)
                if (meta.id) {
                  // folderId → groupId のマイグレーション
                  if (meta.tags) {
      ...
      // キャッシュが存在しなかった場合、走査結果からキャッシュを生成
      if (!cacheLoaded) {
        this.saveMediaCache()
      }

      console.log(`[MediaLibrary] Async Init finished for ${this.path}`)
    } catch (error) {
      console.error(`Failed to load database for ${this.path}:`, error)
    }
  }

  // Detect and fix duplicate IDs in loaded media files
  private async fixDuplicateIds() {
    const idMap = new Map<number, any[]>()
    for (const media of this.db.mediaFiles) {
      if (!idMap.has(media.id)) idMap.set(media.id, [])
      idMap.get(media.id)!.push(media)
    }

    const duplicates = Array.from(idMap.values()).filter(list => list.length > 1)
    if (duplicates.length === 0) return

    console.log(`[MediaLibrary] Found ${duplicates.length} duplicate ID sets. Fixing...`)

    // Ensure nextMediaId is safe
    if (this.db.mediaFiles.length > 0) {
      this.db.nextMediaId = Math.max(this.db.nextMediaId, Math.max(...this.db.mediaFiles.map(ms => ms.id)) + 1)
    }

    // Fix duplicates: keep the first one, re-assign others
    for (const group of duplicates) {
      // Skip the first one (original or arbitrary winner)
      for (let i = 1; i < group.length; i++) {
        const media = group[i]
        const oldId = media.id
        const newId = this.db.nextMediaId++
        media.id = newId

        // Save to disk (metadata.json inside existing uniqueId folder)
        this.saveMediaMetadata(media)
        console.log(`[MediaLibrary] Fixed duplicate ID: ${oldId} -> ${newId} (${media.file_name})`)
      }
    }
  }
  ...
  private rebuildIndices() {
    // 後方互換: 旧形式 parentId(単一値) → 新形式 parentIds(配列) へのマイグレーション
    this.db.mediaFiles.forEach(media => {
      const legacyParentId = (media as any).parentId
      if (legacyParentId != null && legacyParentId !== undefined) {
        if (!media.parentIds) media.parentIds = []
        if (!media.parentIds.includes(legacyParentId)) {
          media.parentIds.push(legacyParentId)
        }
        delete (media as any).parentId
        this.saveMediaMetadata(media)
      }
    })
  ...
  private saveMediaMetadata(media: any) {
    ...
    // 個別メタデータ保存のたびにキャッシュ更新をスケジュール
    this.scheduleMediaCacheSave()
  }

  /**
   * 全メディアのメタデータをライブラリ内の単一キャッシュファイルに書き出す。
   * 起動時に8000件超の個別JSONを読む代わりに、このキャッシュを一括読み込みする。
   */
  private saveMediaCache() {
    try {
      console.log(`[MediaLibrary] Saving media cache (${this.db.mediaFiles.length} items)...`)
      // 整形なしで書き出し: ファイルサイズとI/O時間を最小化
      fs.writeJsonSync(this.mediaCachePath, this.db.mediaFiles)
      console.log(`[MediaLibrary] Media cache saved.`)
    } catch (error) {
      console.error(`[MediaLibrary] Failed to save media cache:`, error)
    }
  }

  /**
   * キャッシュ書き込みをデバウンスでスケジュールする。
   * 連続した変更操作（タグ編集、レート変更など）を5秒間束ねて1回の書き込みにまとめる。
   */
  private scheduleMediaCacheSave() {
  ...
  /**
   * キャッシュファイルから全メディアメタデータを一括読み込みする。
   * 成功時は true を返し、ファイルが存在しないか読み込み失敗時は false を返す。
   */
  private async loadFromMediaCache(): Promise<boolean> {
    try {
      if (!await fs.pathExists(this.mediaCachePath)) {
        return false
      }

      console.log(`[MediaLibrary] Loading from media cache...`)
      const startTime = Date.now()
      const cachedMediaFiles = await fs.readJson(this.mediaCachePath)

      if (!Array.isArray(cachedMediaFiles)) {
        console.warn(`[MediaLibrary] Invalid cache format. Falling back to directory scan.`)
        return false
      }

      this.db.mediaFiles = cachedMediaFiles

      // nextMediaId をキャッシュ内の最大IDから復元
      if (this.db.mediaFiles.length > 0) {
        this.db.nextMediaId = Math.max(...this.db.mediaFiles.map(m => m.id)) + 1
      }

      const elapsed = Date.now() - startTime
      console.log(`[MediaLibrary] Media cache loaded: ${this.db.mediaFiles.length} items in ${elapsed}ms`)
      return true
    } catch (error) {
      console.error(`[MediaLibrary] Failed to load media cache, falling back to directory scan:`, error)
      // キャッシュ破損時はリセットして個別走査にフォールバック
      this.db.mediaFiles = []
      return false
    }
  }
    ...
    return (this.importQueue = this.importQueue.then(async () => {
      ...
      for (let i = 0; i < totalFiles; i++) {
        ...
        try {
          ...
          let id = this.db.nextMediaId
          while (this.db.mediaFiles.some(m => m.id === id)) {
            id++
          }
          this.db.nextMediaId = id + 1
          ...
          report('Metadata...', 0.3)
          const { width, height, duration, artist, description, url, comment } = await getMediaMetadata(destPath)
          ...
          const metadata = {
            ...
            tags: [], folders: [], comments: [] as any[]
          }

          // Add initial comment if present (typically from Niconico URL)
          if (comment) {
            const newComment = {
              id: this.db.nextCommentId++,
              mediaId: id,
              text: comment,
              time: 0,
              nickname: 'System',
   ...
   */
  public async getDuplicatesForMedia(mediaId: number) {
    await this.ready
   ...
   */
  /**
   * ファイルシステム上の孤立したファイル/フォルダをスキャンする
   * DBに登録されていないが、imagesフォルダ内に存在するフォルダやファイルを特定する
   */
  public async scanFileSystemOrphans() {
    await this.ready
    const imagesDir = path.join(this.path, 'images')
    if (!await fs.pathExists(imagesDir)) return []

    const validUniqueIds = new Set(this.db.mediaFiles.map(m => m.uniqueId))
    const orphans: any[] = []

    const dirs = await fs.readdir(imagesDir)
    for (const dir of dirs) {
      if (dir === '.DS_Store' || dir === 'Thumbs.db') continue

      const dirPath = path.join(imagesDir, dir)
      const stats = await fs.stat(dirPath)
      if (!stats.isDirectory()) continue

      // チェック1: DBに存在しないIDのフォルダか？
      if (!validUniqueIds.has(dir)) {
        // フォルダごと孤立
        // 中身の動画ファイルを探して情報を付加
        const files = await fs.readdir(dirPath)
        const videoFiles = files.filter((f: string) => {
          const ext = path.extname(f).toLowerCase()
          return ['.mp4', '.mkv', '.avi', '.mov', '.webm', '.flv', '.wmv', '.mp3', '.wav', '.flac', '.m4a', '.ogg', '.aac', '.wma'].includes(ext)
        })
        ...
        if (videoFiles.length > 0) {
          ...
          for (const file of videoFiles) {
            ...
            orphans.push({
              type: 'folder_orphan',
              path: dirPath, // フォルダパス
              file_path: filePath, // 代表ファイル
              name: file,
              size: fStat.size,
              reason: 'Library entry not found'
            })
          }
        } else {
          // 空、または動画なしの孤立フォルダ (ゴミとして扱う)
          orphans.push({
            type: 'empty_orphan',
            path: dirPath,
            reason: 'Empty or no media'
          })
        }
      }
    }
  ...
  public async deleteFileSystemPath(targetPath: string) {
    await this.ready
    // 安全のため、imagesフォルダ配下であることを確認
    const imagesDir = path.join(this.path, 'images')
    if (targetPath.startsWith(imagesDir)) {
      console.log(`[MediaLibrary] Deleting orphan path: ${targetPath}`)
  ...
  public async addMediaFile(filePath: string, fileName: string, fileType: string, options: any = {}) {
    await this.ready
    ...
    this.db.mediaFiles.push({
      ...
      last_played_at: null, artist: options.artist || null, artists: [], description: options.description || null,
      url: options.url || null, dominant_color: null, tags: [], folders: [], comments: [], parentId: null // Renamed genres -> folders
    } as any)
  ...
  public async getVideosMissingMetadata() {
    await this.ready
    return this.db.mediaFiles.filter((m) => m.file_type === 'video' && (!m.duration || m.duration === 0))
  }

  public async updateVideoMetadata(id: number, width: number, height: number, duration: number) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
    if (media) {
  ...
  public async refreshMetadataForMedia(ids: number[]) {
    await this.ready
    ...
    for (const id of ids) {
      ...
      try {
        // ファイル存在確認
        if (!await fs.pathExists(media.file_path)) {
          console.warn(`[Refresh] File not found: ${media.file_path}`)
          continue
        }
        ...
        media.modified_date = stats.mtime.toISOString()

        if (meta.duration) media.duration = meta.duration
        if (meta.width) media.width = meta.width
        if (meta.height) media.height = meta.height

        // メタデータ（URL、説明、コメント）の更新
        if (meta.url !== undefined) media.url = meta.url
        if (meta.description !== undefined) media.description = meta.description
        if (meta.comment !== undefined) media.comment = meta.comment
        if (meta.artist !== undefined) media.artist = meta.artist

        // サムネイルの再生成（オプションまたは常に？ ここでは「再取得」なので、サムネイルがない場合、または強制的に更新する）
        // 既存の挙動に合わせて、サムネイルが存在しない場合のみ生成するか、それともすべて再生成するか。
        // ユーザーの意図としては「壊れた/古い」情報の更新なので、サムネイルもチェックするのが親切。
        if (!media.thumbnail_path || !await fs.pathExists(media.thumbnail_path)) {
          // サムネイル再生成ロジック
          const thumbPath = await getThumbnailPath(this.path, media.id, media.file_path)
          const mode = config.thumbnailMode || 'speed'
          if (await createThumbnail(media.file_path, thumbPath, mode)) {
            media.thumbnail_path = thumbPath
            try {
              const color = await extractDominantColor(thumbPath)
              if (color) media.dominant_color = color
        ...
        this.addAuditLog({
          action: 'media_refresh_metadata',
          targetId: media.id,
          targetName: media.file_name,
          description: `メタデータを更新しました: ${media.file_name}`
        })

      } catch (e) {
        console.error(`[Refresh] Failed to refresh metadata for ${media.file_name}:`, e)
      }
    }
  }
  ...
  public async get(id: number) {
    await this.ready
    return this.db.mediaFiles.find((m) => m.id === id)
  }
  ...
  public async getAllMediaFiles() {
    ...
    // 相対パスを絶対パスに変換するヘルパー
    const toAbsolutePath = (relativePath: string | null): string | null => {
    ...
    // Media -> Children pre-index (Shallow): parentIds配列に基づいて逆引き
    const childrenMap = new Map<number, any[]>()
    this.db.mediaFiles.forEach(m => {
      const pIds: number[] = m.parentIds || []
      pIds.forEach((pid: number) => {
        ...
        childrenMap.get(pid)!.push({
          ...
          thumbnail_path: toAbsolutePath(m.thumbnail_path)
        })
      })
    })
    ...
    return this.db.mediaFiles.map((media) => {
      ...
      // Resolve Parents (複数親対応: 浅いコピーでサイクルと過大ペイロードを防ぐ)
      const parentIds: number[] = media.parentIds || []
      const parents = parentIds
        .map((pid: number) => mediaMap.get(pid))
        .filter(Boolean)
        .map((p: any) => ({
          id: p.id,
          title: p.title,
          file_name: p.file_name,
          thumbnail_path: toAbsolutePath(p.thumbnail_path)
        }))
      const children = childrenMap.get(media.id) || []

      return {
        ...media,
        thumbnail_path: toAbsolutePath(media.thumbnail_path),
        tags,
        folders,
        parents,
        children
  ...
  public async getMediaFileWithDetails(id: number) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
    ...
    // Resolve Parents (複数親対応)
    const parentIds: number[] = media.parentIds || []
    const parents = parentIds
      .map((pid: number) => this.db.mediaFiles.find(m => m.id === pid))
      .filter(Boolean)
      .map((p: any) => ({ ...p, thumbnail_path: toAbsolutePath(p.thumbnail_path) }))

    // Resolve Children (parentIds配列内に自分のIDを含むメディアを探す)
    const children = this.db.mediaFiles
      .filter(m => (m.parentIds || []).includes(id))
      .map(c => ({ ...c, thumbnail_path: toAbsolutePath(c.thumbnail_path) }))
  ...
  public async updateThumbnail(id: number, thumbnailPath: string) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
    if (media) { media.thumbnail_path = thumbnailPath; this.saveMediaMetadata(media) }
  }

  public async moveToTrash(id: number) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
    if (media) { media.is_deleted = true; this.saveMediaMetadata(media) }
  }

  public async restoreFromTrash(id: number) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
   ...
   */
  public async moveMediaFilesToTrash(ids: number[], isDeleted: boolean) {
    await this.ready
    ids.forEach(id => {
      const media = this.db.mediaFiles.find(m => m.id === id)
  ...
  public async updateLastPlayed(id: number) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
    if (media) {
  ...
  public async updateFileName(id: number, newName: string) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
    if (media) {
  ...
  public async updateArtist(id: number, artist: string | null) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
    if (media) {
      media.artist = artist;
  ...
  public async updateDescription(id: number, description: string | null) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
    if (media) {
  ...
  public async updateUrl(id: number, url: string | null) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === id)
    if (media) {
      media.url = url;
   ...
   * 自己参照や直接的な循環依存を防止する
   */
  public async addParent(childId: number, parentId: number) {
    await this.ready
    ...
    // 既に登録済みの場合はスキップ
    if (media.parentIds.includes(parentId)) return

    // 直接的な循環依存の防止: 親候補が自分を親に持っていないかチェック
    const parent = this.db.mediaFiles.find(m => m.id === parentId)
    if (parent && (parent.parentIds || []).includes(childId)) return

    media.parentIds.push(parentId)
    this.saveMediaMetadata(media)
  }
   ...
   * 親作品を削除する（複数親対応）
   */
  public async removeParent(childId: number, parentId: number) {
    await this.ready
    const media = this.db.mediaFiles.find((m) => m.id === childId)
    if (!media || !media.parentIds) return

    media.parentIds = media.parentIds.filter((id: number) => id !== parentId)
    this.saveMediaMetadata(media)
  }

  public async searchMedia(query: string, targets?: any) {
    await this.ready
    if (!query) return []
  ...
  public async getAllTags() {
    await this.ready
    return this.db.tags.sort((a, b) => a.name.localeCompare(b.name))
  }
  ...
  public async addTagToMedia(mediaId: number, tagId: number) {
    await this.ready
  ...
  public async addTagsToMedia(mediaIds: number[], tagIds: number[]) {
    await this.ready
  ...
  public async removeTagFromMedia(mediaId: number, tagId: number) {
    await this.ready
  ...
  public async updateTagGroup(tagId: number, groupId: number | null) {
    await this.ready
    const tag = this.db.tags.find((t) => t.id === tagId)
    if (tag) {
      tag.groupId = groupId;
  ...
  public async getAllTagGroups() {
    await this.ready
    return this.db.tagGroups.sort((a: any, b: any) => a.name.localeCompare(b.name))
  }

  public async createTagGroup(name: string) {
    await this.ready
  ...
  public async deleteTagGroup(id: number) {
    await this.ready
  ...
  public async renameTagGroup(id: number, newName: string) {
    await this.ready
  ...
  public async createFolder(baseName: string, parentId: number | null = null) {
    await this.ready
    let name = baseName
    let counter = 1
  ...
  public async addFolderToMedia(mediaId: number, folderId: number) {
    await this.ready
    const media = this.db.mediaFiles.find(m => m.id === mediaId)
  ...
  public async removeFolderFromMedia(mediaId: number, folderId: number) {
    await this.ready
  ...
  public async updateFolderStructure(updates: { id: any; parentId: any; orderIndex: number }[]) {
    await this.ready
  ...
  // コメント
  public async addComment(mediaId: number, text: string, time: number, nickname?: string) {
    await this.ready
  ...
  public async getComments(mediaId: number) {
    await this.ready
  ...
  /* public async deleteFileSystemPath(targetPath: string) {
      // 安全のため images 以下のパスであることを確認する
      const imagesDir = path.join(this.path, 'images')
      if (!targetPath.startsWith(imagesDir)) {
        console.error('[Security] Attempt to delete outside of library images folder:', targetPath)
        return false
      }
      try {
        await fs.remove(targetPath)
        ...
        console.error('Failed to delete path:', targetPath, e)
        return false
      }
    } */
}
...
class LibraryRegistry {
  private instances: Map<string, LegacyMediaLibrary> = new Map()

  public getLibrary(libraryPath: string): LegacyMediaLibrary {
    if (!this.instances.has(libraryPath)) {
      this.instances.set(libraryPath, new LegacyMediaLibrary(libraryPath))
    }
...
export function initDatabase() {
  console.log('[Database] initDatabase START')
  loadLibrariesConfig()
  activeLibraryPath = librariesConfig.activeLibraryPath
  console.log('Database initialized')
  console.log('Active library:', activeLibraryPath)
  console.log('[Database] initDatabase END')
}

// アクティブライブラリ取得ヘルパー
export function getActiveMediaLibrary(): LegacyMediaLibrary | null {
...
export const mediaDB = {
  importMediaFiles: async (paths: string[]) => getActiveMediaLibrary()?.importMediaFiles(paths) || Promise.resolve([]),
  addMediaFile: async (path: string, name: string, type: string, opts: any) => getActiveMediaLibrary()?.addMediaFile(path, name, type, opts),
  updateRating: async (id: number, val: number) => getActiveMediaLibrary()?.updateRating(id, val),
  getVideosMissingMetadata: async () => getActiveMediaLibrary()?.getVideosMissingMetadata() || [],
  updateVideoMetadata: async (id: number, w: number, h: number, d: number) => getActiveMediaLibrary()?.updateVideoMetadata(id, w, h, d),
  get: async (id: number) => getActiveMediaLibrary()?.get(id),
  getAllMediaFiles: async () => getActiveMediaLibrary()?.getAllMediaFiles() || [],
  getMediaFileWithDetails: async (id: number) => getActiveMediaLibrary()?.getMediaFileWithDetails(id) || null,
  updateThumbnail: async (id: number, path: string) => getActiveMediaLibrary()?.updateThumbnail(id, path),
  moveToTrash: async (id: number) => getActiveMediaLibrary()?.moveToTrash(id),
  restoreFromTrash: async (id: number) => getActiveMediaLibrary()?.restoreFromTrash(id),
  deletePermanently: async (id: number) => getActiveMediaLibrary()?.deletePermanently(id) || Promise.resolve(),
  updateLastPlayed: async (id: number) => getActiveMediaLibrary()?.updateLastPlayed(id),
  updateFileName: async (id: number, name: string) => getActiveMediaLibrary()?.updateFileName(id, name),
  updateArtist: async (id: number, artist: string | null) => getActiveMediaLibrary()?.updateArtist(id, artist),
  updateDescription: async (id: number, desc: string | null) => getActiveMediaLibrary()?.updateDescription(id, desc),
  updateUrl: async (id: number, url: string | null) => getActiveMediaLibrary()?.updateUrl(id, url),
  moveMediaFilesToTrash: async (ids: number[], isDeleted: boolean) => getActiveMediaLibrary()?.moveMediaFilesToTrash(ids, isDeleted),
  deleteMediaFilesPermanently: async (ids: number[]) => getActiveMediaLibrary()?.deleteMediaFilesPermanently(ids) || Promise.resolve(),
  refreshMetadataForMedia: async (ids: number[]) => getActiveMediaLibrary()?.refreshMetadataForMedia(ids) || Promise.resolve(),
  updateDominantColor: async (id: number, color: string) => getActiveMediaLibrary()?.updateDominantColor(id, color),
  checkDuplicates: async (filePaths: string[]) => getActiveMediaLibrary()?.checkDuplicates(filePaths) || Promise.resolve([]),
  getDuplicatesForMedia: async (mediaId: number) => getActiveMediaLibrary()?.getDuplicatesForMedia(mediaId) || [],
  findLibraryDuplicates: async (criteria?: { name: boolean; size: boolean; duration: boolean; modified: boolean }) => getActiveMediaLibrary()?.findLibraryDuplicates(criteria) || Promise.resolve([]),
  refreshLibraryMetadata: async (onProgress: (c: number, t: number) => void) => getActiveMediaLibrary()?.refreshLibraryMetadata(onProgress) || Promise.resolve(),
  searchMedia: async (query: string, targets?: any) => getActiveMediaLibrary()?.searchMedia(query, targets) || [],
  addParent: async (childId: number, parentId: number) => getActiveMediaLibrary()?.addParent(childId, parentId),
  removeParent: async (childId: number, parentId: number) => getActiveMediaLibrary()?.removeParent(childId, parentId),
  scanFileSystemOrphans: async () => getActiveMediaLibrary()?.scanFileSystemOrphans() || [],
  deleteFileSystemPath: async (path: string) => getActiveMediaLibrary()?.deleteFileSystemPath(path) || false,
}

export const tagDB = {
  getAllTags: async () => getActiveMediaLibrary()?.getAllTags() || [],
  createTag: async (name: string) => getActiveMediaLibrary()?.createTag(name),
  deleteTag: async (id: number) => getActiveMediaLibrary()?.deleteTag(id),
  addTagToMedia: async (mId: number, tId: number) => getActiveMediaLibrary()?.addTagToMedia(mId, tId),
  addTagsToMedia: async (mIds: number[], tIds: number[]) => getActiveMediaLibrary()?.addTagsToMedia(mIds, tIds),
  removeTagFromMedia: async (mId: number, tId: number) => getActiveMediaLibrary()?.removeTagFromMedia(mId, tId),
  updateTagGroup: async (tId: number, gId: number | null) => getActiveMediaLibrary()?.updateTagGroup(tId, gId),
}

export const tagGroupDB = {
  getAllTagGroups: async () => getActiveMediaLibrary()?.getAllTagGroups() || [],
  createTagGroup: async (name: string) => getActiveMediaLibrary()?.createTagGroup(name),
  deleteTagGroup: async (id: number) => getActiveMediaLibrary()?.deleteTagGroup(id),
  renameTagGroup: async (id: number, name: string) => getActiveMediaLibrary()?.renameTagGroup(id, name),
}

export const folderDB = {
  getAllFolders: async () => getActiveMediaLibrary()?.getAllFolders() || [],
  createFolder: async (name: string, pId: number | null) => getActiveMediaLibrary()?.createFolder(name, pId),
  deleteFolder: async (id: number) => getActiveMediaLibrary()?.deleteFolder(id),
  addFolderToMedia: async (mId: number, fId: number) => getActiveMediaLibrary()?.addFolderToMedia(mId, fId),
  removeFolderFromMedia: async (mId: number, fId: number) => getActiveMediaLibrary()?.removeFolderFromMedia(mId, fId),
  renameFolder: async (id: number, name: string) => getActiveMediaLibrary()?.renameFolder(id, name),
  updateFolderStructure: async (updates: any) => getActiveMediaLibrary()?.updateFolderStructure(updates),
}

export const commentDB = {
  addComment: async (mediaId: number, text: string, time: number, nickname?: string) => getActiveMediaLibrary()?.addComment(mediaId, text, time, nickname),
  getComments: async (mediaId: number) => getActiveMediaLibrary()?.getComments(mediaId) || [],
}

export { LegacyMediaLibrary as MediaLibrary };

// Library Management Exports for Main Process
...
export function getActiveLibraryPath() {
  loadLibrariesConfig()
  // Ensure active path is valid
  if (librariesConfig.activeLibraryPath && !librariesConfig.libraries.find(l => l.path === librariesConfig.activeLibraryPath)) {
    librariesConfig.activeLibraryPath = null
    saveLibrariesConfig()
  }
  return librariesConfig.activeLibraryPath
}

export function setActiveLibraryPath(path: string | null) {
  librariesConfig.activeLibraryPath = path
  saveLibrariesConfig()
}

export function addLibraryToConfig(name: string, path: string) {
  loadLibrariesConfig()
  if (!librariesConfig.libraries.some(l => l.path === path)) {
    librariesConfig.libraries.push({
      name,
      path,
      createdAt: new Date().toISOString()
    })
