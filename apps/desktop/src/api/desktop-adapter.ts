...
export class DesktopAdapter implements IMediaLibraryAPI {
    ...
    async addMediaParent(childId: number, parentId: number): Promise<void> {
        return this.api.addMediaParent(childId, parentId);
    }

    async removeMediaParent(childId: number, parentId: number): Promise<void> {
        return this.api.removeMediaParent(childId, parentId);
    }
